...
为了能够有效提高DSP系统的性能，美团平台引入了一种带有清退机制的缓存结构LruCache(Least Recently Used Cache)，在目前的DSP系统中，使用`LruCache + 键值存储数据库（Redis）`用来的机制将远端数据变为本地缓存数据，不仅能够降低平均获取信息的耗时，而且通过一定的清退机制，也可以维持服务内存占用在安全区间。
...
![引入HashLruCache前后TP999耗时](./pic/7523b036.png)
...
![HashLruCache分片数量耗时](./pic/fb89f8c3.png)
